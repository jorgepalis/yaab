...
class ControlCalificaciones(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for usuario in usuarios:
            suma_total += (usuario.valor_numerico_edad or 0) + (
                usuario.valor_numerico_estado_civil or 0) + (usuario.valor_numerico_empleo or 0)
...
class ViewRegistroCreditos(ListView):
    ...
    def get_queryset(self):
        ...
        registro_usuarios = RegistroCreditos.objects.filter(
            cliente__solicitud=True)
...
class ViewCreditosStatusEstatico(ListView):
    ...
    def get_queryset(self):
        ...
        estatus_usuarios = SimuladorPrueba.objects.filter(
            usuario_user__in=usuarios_con_solicitud_true)
...
class ViewCreditosStatus(ListView):
    ...
    def get_queryset(self):
        ...
        for simulador in estatus_usuarios:
            ...
            identificador_prestamo = IdentificadorDePago.objects.filter(
                simulador_relacionado=simulador).order_by('id')
    ...
    def post(self, request, *args, **kwargs):
        ...
        simulador_numero_contrato = request.POST.get('simulador')
        # Obtener la instancia de SimuladorPrueba correspondiente al número de contrato
        simulador = SimuladorPrueba.objects.get(
            identificador_unico=simulador_numero_contrato)
    ...
    #
                ...
                simulador = request.POST.get('simulador')
...
class RegistroPagos(CreateView):
    ...
    def form_valid(self, form):
        ...
        ultimo_pago = RegistroPagosModel.objects.filter(
            simulador=simulador).order_by('-fecha_pago').first()
        numero_pago = 1 if not ultimo_pago else ultimo_pago.numero_pago + 1
        # Calcular el monto restante
        monto_restante = simulador.nombre_prestamo.pago_mensual * \
            numero_pago - form.cleaned_data['monto_pagado']
