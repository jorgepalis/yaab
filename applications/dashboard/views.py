...
from .models import Productos,Plazo,Simulador,ImageYaab,ProductoCreditoGrupal,Prestamo,SimuladorPrueba,PruebaSimula,Dato
from .forms import ProductosForm,PlazoPagoForm,SimuladorForm,FormProductosGrupal,PrestamoForm,SimPruebas,SimuladorPruebaForm,PruebaSimulaForms,DatoForm
...
class DatoView(ListView):
    template_name = 'dashboard/csv.html'
    model = Dato
    context_object_name = 'obj'
    
def subir_csv(request):
    template_name = "dashboard/upload_csv.html"
    ...
    if request.method == "POST":
        ...
        for line in lines:
            ...
            if len(fields)>1:
                ...
                data_dict["employees"] = fields[12]
                
                try:
                    form = DatoForm(data_dict)
                    if form.is_valid():
                        form.save()
                    else:
                    ...
                    print(repr(ex))
        return redirect("dashboard_app:csv")
