...
from applications.users.models import User, RegistroCreditosModel
import uuid
...
class RegistroCreditos(models.Model):
    cliente = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="cliente", blank=True, null=True)
    numero_contrato = models.CharField(
        max_length=12, unique=True, blank=True, null=True)  # Agregado este campo
...
class EstatusCredito(models.Model):
    ...
    cliente_estatus = models.ForeignKey(
        SimuladorPrueba, on_delete=models.CASCADE, related_name="cliente_estatus", blank=True, null=True)
    numero_contrato_estatus = models.ForeignKey(
        RegistroCreditos, on_delete=models.CASCADE, related_name="contrato", blank=True, null=True)
    monto_total = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    desembolso = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    tipo = models.CharField(max_length=12, unique=True, blank=True, null=True)
    monto_pago_men = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    ...
    monto_morosidad = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    saldo_mas_morosidad = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
...
def actualizar_numero_contrato_estatus(sender, instance, created, **kwargs):
    if created:
        ...
        estatus_credito, created = EstatusCredito.objects.get_or_create(
            cliente_estatus=instance.cliente)
...
class RegistroPagosModel(models.Model):
    ...
    # simulador = models.CharField(max_length=50, blank=True, null=True)
    simulador = models.ForeignKey(SimuladorPrueba, on_delete=models.CASCADE,
                                  related_name='simulador_unico', blank=True, null=True)
    monto_pagado = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
...
class RegistroPagosModelManual(models.Model):

    simulador = models.ForeignKey(RegistroCreditosModel, on_delete=models.CASCADE,
                                  related_name='simulador_unico_manual', blank=True, null=True)
    monto_pagado = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    comprobante_pago = models.FileField(
        upload_to='media/', blank=True, null=True)
...
class IdentificadorDePago(models.Model):
    simulador_relacionado = models.ForeignKey(SimuladorPrueba, on_delete=models.CASCADE,
                                              related_name='identificador_pago', blank=True, null=True)
    monto_a_pagar = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    monto_restante = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    periodo_a_pagar = models.IntegerField(blank=True, null=True)
    fecha_oportuna = models.DateTimeField(null=True, blank=True)
    estatus = models.BooleanField(default=False)


class IdentificadorDePagoRegistroMan(models.Model):
    simulador_relacionado = models.ForeignKey(RegistroCreditosModel, on_delete=models.CASCADE,
                                              related_name='identificador_pago_manual', blank=True, null=True)
    monto_a_pagar = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    monto_restante = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
