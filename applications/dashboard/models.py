...
class Prestamo(models.Model):
    tipo_prestamo = models.ForeignKey(
        TipoPrestamo, on_delete=models.CASCADE, blank=True, null=True)
    nombre_producto = models.CharField(max_length=50, blank=True, null=True)
    monto = models.IntegerField(blank=True, null=True)
    tipo_periodo = models.ForeignKey(
        Periodo, on_delete=models.CASCADE, blank=True, null=True)
    plazo = models.IntegerField(blank=True, null=True)  # "mensual" o "semanal"
    interes_ordinario = models.DecimalField(
        max_digits=5, decimal_places=2, blank=True, null=True)
    interes_moratorio = models.DecimalField(
        max_digits=5, decimal_places=2, blank=True, null=True)
    pago_mensual = models.DecimalField(
...
class PruebaSimula(models.Model):
    montos_simulador = models.ForeignKey(
        Prestamo, on_delete=models.CASCADE, related_name='montos_simulador', null=True, blank=True)
...
class SimuladorPrueba(models.Model):
    nombre_prestamo = models.ForeignKey(
        Prestamo, on_delete=models.CASCADE, related_name='tipo_credito', null=True, blank=True)
    usuario_user = models.ForeignKey(
        'users.User', on_delete=models.CASCADE, related_name='usario_user', null=True, blank=True)
    solicitud = models.BooleanField(default=False, blank=True, null=True)
    fecha_registro = models.DateField(
        default=timezone.now, blank=True, null=True)
    # cambio de datatimefield a datafield
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.identificador_unico:
            ...
            ultimo_identificador = SimuladorPrueba.objects.latest(
                'id').identificador_unico if SimuladorPrueba.objects.exists() else 0
            if ultimo_identificador:
                # Obtener el número en aumento del último identificador
                # Obtener los últimos dos dígitos del identificador
                ultimo_numero = int(ultimo_identificador[8:])
            else:
                ultimo_numero = 0
            # Incrementar el número en aumento
            # Limitar a dos dígitos (0-99)
            nuevo_numero = (ultimo_numero + 1) % 100
...
class Plazo(models.Model):
    ...
    interes_credito = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    interes_moratorio = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
...
class Simulador(models.Model):
    tipo_credito = models.ForeignKey(
        Productos, on_delete=models.CASCADE, related_name='tipo_credito', null=True, blank=True)
    # tipo_credito = models.ForeignKey(Plazo, on_delete=models.CASCADE,related_name='tipo_credito',null=True,blank=True)
    plazo_nombre = models.ForeignKey(
        Plazo, on_delete=models.CASCADE, related_name='nombre_plazo', null=True, blank=True)
    term = models.IntegerField(null=True, blank=True)
    amount = models.IntegerField(null=True, blank=True)
    interest_rate = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
    interest_moratorio = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True)
