...
from applications.dashboard.models import ProductoCreditoGrupal, Simulador, SimuladorPrueba, Prestamo
...
def guardar_ubicacion(request):
    if request.method == 'POST':
        # Obtén estos valores del request POST
...
def correo_grupo_credito(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            participantes = ProductoCreditoGrupal.objects.get(
                pk=participantes_id)
        ...
        if participantes:
            ...
            correo_nuevo.guardar_correos(correos_electronicos, nombres, apellidos,
                                         curp, rfc, celular, participantes, monto, correo_coordinador)
            ...
            for correo_destinatario in correos_electronicos:
                ...
                if participantes:
                    ...
                    monto_valor, creado = ProductoCreditoGrupal.objects.get_or_create(
                        ...
                        # Valores adicionales para la creación
                        defaults={'monto_credito': monto}
                    )
...
class FormularioCreditoGrupal(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if signature_data:
            ...
            signature_img = ContentFile(base64.b64decode(
                imgstr), name=f'{self.object.token}_signature.{ext}')

            # Guardar la imagen en el campo firma_imagen
            form.instance.firma_imagen = signature_img
            # Limpiar el campo firma_digital, ya que ahora tienes la imagen
            form.instance.firma_digital = signature_data
...
class RegistrarCreditoView(CreateView):
    ...
    def form_valid(self, form):
        # Guardar la instancia del formulario sin guardarla en la base de datos todavía
        instancia_simulador = form.save(commit=False)

        # Asignar los valores de usuarios y producto a los campos correspondientes
        instancia_simulador.usuario_user = form.cleaned_data['usuarios']
        instancia_simulador.nombre_prestamo = form.cleaned_data['producto']

        # Obtener la fecha del formulario
        fecha_creacion = form.cleaned_data['fecha_credito']

        # Asignar la fecha obtenida al campo fecha_registro
        instancia_simulador.fecha_registro = fecha_creacion

        # Guardar la instancia modificada
        instancia_simulador.save()
